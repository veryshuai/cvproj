...
# cit_times_df = cit_times_df[cit_times_df['cit_times'] <= last_yr + 1]
# 
...
# combined['isCiter']     = pd.notnull(combined['cit_times'])
# 
...
# 
# # APPEND ONTO AUT_PAN
# aut_pan  = aut_pan.append(combined).sort_index(by = ['au','date'])
# aut_pan  = aut_pan[['au','date','dep','start_times','end_times','cit_times','tot_cits','isCiter']].set_index('au')
# fillcols = aut_pan.reset_index().groupby('au').fillna(method = 'ffill')
# aut_pan  = fillcols.set_index('au')
# fillcols = aut_pan.reset_index().groupby('au').fillna(method = 'bfill')
# aut_pan  = fillcols.set_index(['au'])
# aut_pan  = aut_pan.reset_index().drop_duplicates(cols = ['au','date'],take_last = True).set_index(['au','date'])
# 
# combined.to_pickle('combined.pickle')
# aut_pan.to_pickle('aut_pan.pickle')
# 
...
dep_list = aut_pan[['dep','dep_qual','dmean']].drop_duplicates().fillna(value=3)
dep_list.to_pickle('dep_list.pickle')

# CLEAN UP AUTPAN
aut_pan           = aut_pan[['au', 'date', 'dep', 'qual', 'dep_qual', 'isField',
