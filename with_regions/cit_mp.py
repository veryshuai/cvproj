...
class Task(object):
    def __init__(self, cp, dy, dr, l, lp,
             c, fc, nc):
        self.cp = cp
        self.dy = dy 
        self.dr = dr 
    ...
    def __call__(self):
        cl_res, fc_res, nc_res =\
                cit_calc(self.cp, self.dy, self.dr, self.l,
...
def call_parallel(cit_params, dep_year, dep_reg, lp,
    ...
    # Enqueue jobs
    for l in range(4):
        tasks.put(Task(cit_params, dep_year, dep_reg, l, lp,
             citers, first_cits, nocits))
...
def cit_calc(cit_params, dep_year, dep_reg, lat, lp,
             citers, first_cits, nocits):
    ...
    try: 
        cl_res = citers.groupby('au')\
                    .apply(lambda x: cd.cit_lik_cit(alp,
                           bet, gam, x, dep_year, dep_reg, lat, qp))
