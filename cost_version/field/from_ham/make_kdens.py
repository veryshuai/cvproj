...
mkplt(params['alp0']         ,4,4,1,r'base learn $\alpha_{nf}$'       ,0,False)
mkplt(params[' alp1']         ,4,4,2,r'base learn $\alpha_{f}$'       ,0,False)
mkplt(params[' bet0']        ,4,4,3,r'depend learn $\beta_{nf}$'     ,0,False)
mkplt(params[' bet1']        ,4,4,4,r'depend learn $\beta_{f}$'     ,0,False)
mkplt(params[' gam0']       ,4,4,5,r'interest prob $\gamma_{NF}$'      ,0,False)
mkplt(params[' gam1']       ,4,4,6,r'interest prob $\gamma_{F}$'    ,0,False)
mkplt(params[' field_co']   ,4,4,7,r'field wage $\xi_f$'  ,0,False)
mkplt(params[' lat_co']     ,4,4,8,r'lat wage $\xi_l$'    ,0,False)
mkplt(params[' qual_co']    ,4,4,9,r'qual wage $\xi_q$'   ,0,False)
mkplt(params[' lo']         ,4,4,10,r'move cost C',0,False)
# mkplt(params[' lat_prob1']  ,3,4,9,r'init_cond base'   ,0,False)
mkplt(params[' lat_prob1']  ,4,4,11, r'init_cond $\phi_Q$'   ,0,False)
mkplt(params[' lat_prob2']  ,4,4,12, r'init_cond $\phi_F$'   ,0,False)
mkplt(params[' lat_prob3']  ,4,4,13,r'lat disp $\sigma$'   ,0,False)
mkplt(params[' ip']         ,4,4,14,r'log exog $\xi_{ex}$'     ,0,False)
plt.savefig('params_dists_split.png',bbox_inches=0)
# fig2 = plt.figure(
