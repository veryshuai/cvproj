...
def mkplt(x,sp1,sp2,sp3,y_name,col='-b',inc_dat=False,tot=False):
    ...
    for name in x.columns:
        ...
        twentyfive.append(x[name].quantile(q=0.05))
        seventyfive.append(x[name].quantile(q=0.95))
...
# ADD FIRST YEAR OF ZEROS
spreadl['cff'] = 0
spreadl['dff'] = 0
spreadl['sdf'] = 0

fig1 = plt.figure(figsize=(17,  8),  dpi=80)
m2, mean2, med2, dat, lag = mkplt(spreadl[['cff','cf0','cf1','cf2','cf3','cf4']],1,3,1,r'% authors','y',True,1)
lgd = plt.legend([mean2[0], dat[0], lag[0]], ['bench', 'region', 'field_dep', 'data', 'data lag'], 4)
#plt.savefig('sim_plots_cf_cf.png',bbox_inches=0)

# fig5 = plt.figure(figsize=(17,  8),  dpi=80)
m2, mean2, med2, dat, lag = mkplt(spreadl[['dff','df0','df1','df2','df3','df4']],1,3,2,r'% departments','y',True,2)
lgd = plt.legend([mean2[0], dat[0], lag[0]], ['bench', 'region', 'field_dep', 'data', 'data lag'], 4)
...
m2, mean2, med2, dat, lag = mkplt(spreadl[['sdf','sd0','sd1','sd2','sd3','sd4']],1,3,3,r'department coef of var','y',True,3)
lgd = plt.legend([mean2[0], dat[0], lag[0]], ['bench', 'region', 'field_dep', 'data', 'data lag'], 1)
